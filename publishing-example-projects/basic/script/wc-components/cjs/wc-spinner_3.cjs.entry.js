...
const StockFinder = class {
  ...
  render() {
    let content = index.h("ul", null, this.searchResults.map(row => index.h("li", { onClick: this.onSelectSymbol.bind(this, row.symbol) }, index.h("strong", null, row.symbol), " - ", row.name)));
    if (this.loading)
      content = index.h("wc-spinner", null);
    return [
      index.h("form", { onSubmit: this.onFindStocks.bind(this) }, index.h("input", { id: "stock-symbol", type: "text", ref: el => this.stockNameInput = el }), index.h("button", { type: "submit" }, "Find!")),
      content
    ];
  }
};
...
const StockPrice = class {
  ...
  __stencil_render() {
    ...
    return [
      index.h("form", { onSubmit: this.onFecthStockPrice.bind(this) }, index.h("input", { id: "stock-symbol", type: "text", ref: el => this.stockInput = el, value: this.stockUserInput, onInput: this.onUserInput.bind(this) }), index.h("button", { type: "submit", disabled: !this.stockInputValid || this.loading }, "Fetch")),
...
//# sourceMappingURL=wc-spinner_3.cjs.entry.js.map